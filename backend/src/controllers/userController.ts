...
export const register = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = new User({ firstName, lastName, email, password, userType, username,coursesRegistered, semester, studentID, comments, rating });
  await user.save();
  res.status(201).json({
...
export const rateU = asyncHandler(async (req:Request, res:Response) =>{
  const user = await User.findById({ _id: req.params.id }).select("-rating");
  if(!user) {
    res.status(404);
    throw new Error("User not found");
  }
